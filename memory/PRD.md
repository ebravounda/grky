...
---
## 2026-07-30 (i) · Carrusel de banners del inicio gestionable desde el panel + fix alta TV
- **Fix alta TV en Likes** (`likes_sync.py` + `server.py`): (1) `sync_alta_to_likes` ahora es IDEMPOTENTE al crear el cliente — si Likes devuelve error (p.ej. "el cliente ya existe") NO aborta, continúa a signupv2 (igual que create_order). (2) El mensaje de error ahora muestra el detalle real de Likes (última línea del log), no solo el código. (3) **TV**: `_products_payload` y `create_order` envían `lineNumber = email del cliente` (Likes lo exige como email de suscripción) y SIN `portability` — antes iba vacío → "EMAIL PARA SUBSCRIPCIÓN DE TV INVÁLIDO". Formato según ejemplo oficial: `{"family":"TV","productId":"…","lineNumber":"cliente@email.com"}`. ⚠️ El cliente debe tener email válido. No validable en preview (Likes MOCK); validar en VPS.
- **Carrusel de banners del inicio (NUEVO, gestionable desde admin)**:
  - Backend: `DEFAULT_SITE_CONTENT["heroBanners"] = []` ([{url, link, active}]). Nuevo endpoint `POST /api/admin/site-content/upload-banner` {imageData(dataURL)} → guarda con kind "promo" y devuelve `{url: /api/public/promo-image/{id}}` (servido por el endpoint público existente). El PUT de site-content ya persiste heroBanners (shallow $set).
  - Frontend público (`PublicCatalog.js`): si hay banners activos, el inicio muestra un **carrusel de banners a pantalla completa (solo imagen, sin texto encima)**, cuadrado 1:1, rounded-[2rem], auto-rotación 4,5s, puntitos, imagen clicable (link opcional) + 2 CTAs (Ver tarifas / Comprobar fibra). Si NO hay banners, se muestra el hero por defecto (texto + carrusel de fotos). Helper `imgSrc` prefija REACT_APP_BACKEND_URL a URLs `/api/...`.
...
### Iteración 2026-06 (fork) — "Pagar cuenta" público (pago de facturas sin login) [testing_agent iter17: 100%]
- **Pedido del usuario**: botón "Pagar cuenta" en la web principal → el cliente introduce su DNI/CIF/Pasaporte o nº de línea → ve el importe pendiente de sus facturas → paga por Stripe → el admin ve los pagos recibidos.
- **Backend (ya existía, verificado)**: `POST /api/public/account/lookup` {query} → resuelve cliente por fiscalId o lineNumber (`_resolve_customer_by_query`) y devuelve {fiscalId, name, pendingTotal, count, invoices[]} (facturas `status:"pending"`). `POST /api/public/account/checkout` {query, origin_url} → Stripe Checkout modo `payment` por el total pendiente (metadata kind=`public_account`, invoice_ids[]). `GET /api/admin/payments` → pagos exitosos (payment_transactions `payment_status:"paid"`).
- **Backend (añadido)**: el webhook `checkout.session.completed` ahora marca como pagadas TODAS las facturas del array `invoice_ids` cuando `kind==public_account` (antes solo el fallback del polling `/payments/status` lo hacía; ahora es robusto aunque el usuario cierre la pestaña).
- **Frontend (NUEVO)**: página pública `pages/public/PayBill.js` en ruta `/pagar-cuenta` (data-testid `paybill-page`): input `paybill-query`, botón `paybill-lookup`, tarjeta `paybill-result` (saludo + `paybill-total` + lista `paybill-inv-{i}`), botón `paybill-pay` (redirige a Stripe con `window.location.href`), estado "¡Estás al día!" si no hay deuda (`paybill-uptodate`). Estética marca (azul #015EEF, naranja #FF7A00).
- **Frontend (cableado)**: botón "Pagar cuenta" en el header de `PublicCatalog.js` desktop (`header-paybill-btn`) y móvil (`mobile-paybill-btn`) → `/pagar-cuenta`. Panel admin `Payments.js` (ya existía pero NO estaba enrutado): añadida ruta `/app/payments` (perm `billing.manage`) en `App.js` + ítem nav "Pagos recibidos" (`nav-pagos`, icono CreditCard) en `AdminLayout.js`. `PaymentResult.js`: soporta usuario anónimo (botón "Volver al inicio" → `/`) y mensaje de éxito adecuado para pago múltiple ("Tus N factura(s) han quedado marcadas como pagadas").
- **Verificado (testing_agent iter17, 100% backend + frontend, 0 issues)**: lookup DNI 12345678A → Juan, 352.42 €, 12 facturas; lookup por nº de línea → mismo cliente; NOEXISTE999 → 404 + toast; `paybill-pay` redirige a checkout.stripe.com real (test); admin login OK, "Pagos recibidos" carga (estado vacío 0.00 €). Deuda técnica anotada (no bloqueante): server.py >2800 líneas (modularizar); PublicCatalog.js grande.
- **DESPLIEGUE VPS** (cambió backend + frontend): `cd /opt/goroky && git pull && cd frontend && NODE_OPTIONS=--max-old-space-size=2048 yarn build && rm -rf /var/www/vhosts/rokymovil.com/a.rokymovil.com/* && cp -r build/* /var/www/vhosts/rokymovil.com/a.rokymovil.com/ && sudo systemctl restart goroky-api.service` (conservar `.htaccess` SPA). Config Stripe (test/live) y webhook `/api/billing/webhook` (o `/api/stripe/webhook`) desde Ajustes.


### Iteración 2026-06 (fork) — Monitor del servidor (solo super admin) [testing_agent iter18+19: 100%]
- **Pedido del usuario**: función en el menú, visible SOLO para él como admin, para monitorear el servidor: CPU/RAM/disco/uptime, servicios del sistema, integraciones, qué dominio consume más recursos (detección de ataques web), log de modificaciones de archivos de todos los dominios, acceso al panel de Plesk, consumo/estado de bases de datos (MongoDB + MySQL/MariaDB, solo lectura), y todos los servicios de sus apps del VPS (tramilex, goroky, ingresoqr, gym24, mvg) + puertos.
- **Restricción de acceso**: `require_superadmin` (backend) permite solo si `role==admin` Y email ∈ `MONITOR_ADMIN_EMAILS`/`ADMIN_EMAIL`. En preview ADMIN_EMAIL=admin@goroky.com; en VPS = soporte@goroky.com → solo ese email ve el monitor. Nav condicional: `GET /api/admin/monitor/access` → `AdminLayout` muestra el ítem "Monitor servidor" (nav-monitor) solo si allowed.
- **Backend nuevos módulos**: `server_monitor.py` (psutil: cpu/mem/swap/disco/uptime/red/top procesos; `service_status` systemd; `domain_traffic` parsea access logs por vhost → peticiones/errores/top IPs + heurística de ataque; `domain_disk_usage` (du por dominio); `recent_file_changes` escáner de mtimes en vhosts). `db_monitor.py` (`mysql_stats` vía pymysql solo-lectura: tamaños de BDs por information_schema + estado global; auto-detecta clave root de Plesk en `/etc/psa/.psa.shadow`; `app_services` agrupa systemd por app; `listening_ports` vía ss/netstat). `plesk_client.py` (API REST v2: `list_domains`, `cli_call`, `domain_info`, `login_link` de un solo uso; auth X-API-Key contra https://127.0.0.1:8443).
- **Endpoints** (`/api/admin/monitor/*`, todos require_superadmin): `access`, `system` (métricas+servicios+integraciones+mongo ping), `databases` (MongoDB en vivo vía motor: listDatabases+serverStatus + MySQL), `services` (apps+running+puertos), `domains` (tráfico+disco), `file-changes?hours=`, `plesk/domains`, `plesk/login-link`, `plesk/config` GET/PUT (guarda en app_settings `_id:"monitor"`, enmascara pleskApiKey y mysqlPassword; incluye appKeywords).
- **Frontend**: `pages/admin/ServerMonitor.js` (ruta `/app/monitor`), 6 pestañas: Sistema (gauges + discos + servicios + integraciones + top procesos, auto-refresh 5s), Bases de datos (tarjetas MongoDB/MySQL), Servicios & Apps (tarjeta por app tramilex/goroky/ingresoqr/gym24/mvg + servicios activos + puertos), Dominios & Ataques, Cambios de archivos (selector de horas), Plesk & MySQL (config API key Plesk + credenciales MySQL + "Abrir panel de Plesk"). data-testids: monitor-page, monitor-tab-{system,databases,services,domains,files,plesk}, gauge-*, db-mongo, db-mysql, app-{kw}, port-{i}, plesk-host, plesk-apikey, mysql-host/port/user/pw, plesk-save, plesk-open-panel.
- **Comportamiento en preview** (esperado): MongoDB SÍ da datos reales (v7.0.40); MySQL/systemd/logs/vhosts/Plesk degradan con `available:false` + nota (no rompen). En el VPS mostrarán datos reales.
- **Dependencias nuevas**: `psutil`, `pymysql` (en requirements.txt). ⚠️ Tras cada `pip freeze` reaparece la línea `litellm @ https://...whl` que hay que BORRAR de requirements.txt (choca con emergentintegrations en el VPS) — ya eliminada.
- **DESPLIEGUE VPS** (cambió backend + frontend): `cd /opt/goroky && git pull && cd backend && venv/bin/pip install psutil pymysql && cd ../frontend && NODE_OPTIONS=--max-old-space-size=2048 yarn build && rm -rf /var/www/vhosts/rokymovil.com/a.rokymovil.com/* && cp -r build/* /var/www/vhosts/rokymovil.com/a.rokymovil.com/ && sudo systemctl restart goroky-api.service`.
  - ⚠️ **Permisos**: para leer logs/disco de `/var/www/vhosts`, `systemctl`, `ss` y `/etc/psa/.psa.shadow` (clave MySQL root de Plesk), el servicio `goroky-api.service` debe ejecutarse como **root** (o con ACLs). Si no, esas secciones degradan a "no disponible".
  - **Plesk API key**: generar en el VPS con `curl -k -X POST --user 'admin:CLAVE' -d '{}' "https://127.0.0.1:8443/api/v2/auth/keys"` → pegar la `key` en la pestaña "Plesk & MySQL".
  - **MongoDB prod**: `listDatabases`/`serverStatus` requieren que el usuario de Mongo tenga rol de lectura/monitor; si falla, la tarjeta MongoDB mostrará una nota.

### Iteración 2026-06 (fork) — Monitor: motivos del ataque + abrir archivo en Plesk
- **Detalle "posible ataque"** (`server_monitor.domain_traffic`): ahora devuelve `reasons[]` (por qué), `severity` (ok/medium/high), `notFound`, `sensitivePaths[]`. Heurísticas: IP que concentra >40% y >300 hits (fuerza bruta/DoS), tasa de errores >50%, 404 >40% (escáner), y >20 peticiones a rutas sensibles (regex `_SENSITIVE`: wp-login.php, xmlrpc.php, /.env, /.git, /phpmyadmin, /administrator, shells, /vendor, etc.). Verificado por unit test (evil.com → high + 4 motivos; normal.com → ok).
- **Frontend** (pestaña Dominios & Ataques): fila clicable que expande (`traffic-detail-{i}`) mostrando motivos, Top IPs, rutas más pedidas, códigos de estado y rutas sensibles. Badge "Ataque probable" (high, rojo) / "Posible ataque · ver" (medium, ámbar). data-testid `attack-{i}`.
- **Abrir archivo en Plesk** (`POST /api/admin/monitor/plesk/file-link` {domain, path}): resuelve el id del dominio vía `list_domains` y construye la URL del Administrador de Archivos `{{host}}/smb/file-manager/list/domainId/{{id}}?currentDir=<carpeta url-encoded>` (carpeta = dirname del archivo, relativa al home del dominio quitando el prefijo `<dominio>/`). Botón "Revisar en Plesk" (`change-plesk-{i}`) en cada fila de Cambios de archivos → abre esa carpeta en Plesk. Sin Plesk configurado → 503 (toast).
...
### Iteración 2026-06 (fork) — Botones "Cobrar ahora tarjeta / SEPA"
- Nuevos botones en la ficha del cliente (`CustomerDetail.js`): **Cobrar ahora tarjeta** y **Cobrar ahora SEPA** (data-testid `charge-now-card-btn` / `charge-now-sepa-btn`). Abren un diálogo (`charge-now-dialog`) que PREGUNTA el importe cada vez (precargado con la cuota mensual = suma PVP de líneas activas vía `_resolve_monthly`) y el concepto (precargado con el nombre de la tarifa, editable). Avisa en rojo si el cliente no tiene el método guardado.
- Backend (`server.py`): `GET /api/customers/{fiscalId}/charge-suggestion` → {amount, concept, cardOnFile, sepaOnFile}. `POST /api/customers/{fiscalId}/charge-now` {amount, method, concept} → cobro off_session inmediato con el método GUARDADO. Tarjeta: PaymentIntent confirm=True → factura "paid" al instante. SEPA: PaymentIntent payment_method_types=["sepa_debit"] off_session con el mandato guardado → status "processing" → factura "pending"+chargeStatus="processing" (liquida en días). Helper `_get_saved_pm_typed(customer, method)` (card/sepa_debit). Si no hay método guardado → 400 pidiendo usar "Enviar enlace tarjeta"/"Enviar SEPA" primero.
- Webhook Stripe ampliado: `payment_intent.succeeded` marca la factura `paid` (por `stripePaymentIntentId`) — necesario para que el adeudo SEPA cierre la factura cuando liquide; `payment_intent.payment_failed` → chargeStatus="failed".
- Verificado por curl (suggestion OK; charge-now sin método → 400 correcto) + screenshot (botones + diálogo SEPA con aviso). El cobro real off_session con método guardado no es E2E-testeable en preview sin una tarjeta/mandato guardado (misma limitación que el resto de cobros off_session), pero replica el patrón ya funcionando de `/charge` y `/invoices/{id}/charge`.
- DESPLIEGUE VPS (backend + frontend): git pull + yarn build + copiar build + restart backend. El webhook de prod debe tener habilitados los eventos payment_intent.succeeded / payment_intent.payment_failed en Stripe.

### Iteración 2026-06 (fork) — Etiqueta SEPA en proceso + Cobro masivo del mes (tarjeta+SEPA)
- **Etiqueta "SEPA en proceso"** (`CustomerDetail.js`): en cada fila de factura, si `status!="paid"` y `chargeStatus=="processing"` se muestra un badge azul "SEPA en proceso · liquida en unos días" (data-testid `inv-sepa-processing-{invoiceNumber}`). El cierre a "paid" llega por el webhook `payment_intent.succeeded`.
- **Cobro masivo del mes** (`Billing.js`, botón verde `charge-all-btn` "Cobrar todo el mes"): amplío `POST /api/billing/charge-all-pending` para cobrar TODAS las facturas pendientes con el método guardado de cada cliente. Antes solo tarjeta (`_get_saved_pm`); ahora usa `_get_saved_pm_typed` según `recurring.method` (con fallback al otro tipo): tarjeta→cobro inmediato (paid), SEPA→adeudo off_session (`payment_method_types=["sepa_debit"]`)→status "processing" (factura queda pending+chargeStatus=processing, con `stripePaymentIntentId` para que el webhook la cierre al liquidar). Resultado ahora incluye `processing`. Toast: "X cobradas · Y en proceso (SEPA) · Z fallidas · W sin método".
- Verificado: curl charge-all-pending (52 pendientes → 52 skipped en preview, sin métodos guardados) + screenshots (botón verde en Cobros). El cobro real off_session no es E2E-testeable en preview sin métodos guardados.
- DESPLIEGUE VPS (backend + frontend): git pull + yarn build + copiar build + restart backend.

### Iteración 2026-06 (fork) — Eliminar solicitudes de alta
- **Botón papelera en cada fila de Solicitudes** (`Solicitudes.js`, data-testid `delete-btn-{fiscalId}`) con confirmación. Disponible para cualquier solicitud (pendiente, aprobada, corrección…).
- Backend `POST /api/applications/{token}/delete` (require_admin): borra el documento de la solicitud. Si tiene una orden LOCAL asociada (por `contractNumber`) que NO se ha aprovisionado en Likes (`likesOrderId` vacío y status != COMPLETED/ACTIVE), borra también esa orden y sus facturas no pagadas (contractNumber, status != paid). Nunca toca órdenes ya creadas en Likes. Devuelve {orders, invoices} borradas.
- Verificado por curl: delete token inexistente → 404; borrado real de una solicitud TEST → 200 {ok, orders:0, invoices:0}, lista pasa de 13 a 12. Screenshot: papelera en todas las filas.
- DESPLIEGUE VPS (backend + frontend): git pull + yarn build + copiar build + restart backend.

### Iteración 2026-06 (fork) — Domiciliación SEPA desde el área de clientes
- **Portal cliente** (`ClientDashboard.js`): nueva tarjeta "Domiciliación bancaria (SEPA)" (data-testid `sepa-card`) que muestra el IBAN enmascarado (ES91 •••• •••• 1332 · domiciliado) si ya está domiciliado, o un aviso si no. Botón "Domiciliar"/"Cambiar IBAN" (`sepa-setup-btn`) abre diálogo (`sepa-dialog`) con campo IBAN; al continuar redirige a la pasarela segura de Stripe para firmar el mandato.
- **Backend** `POST /api/me/sepa-setup` (cliente autenticado, body {iban, origin_url}): guarda `customer.iban` + `paymentMethod=SEPA CORE` (para visualizarlo en portal Y en CRM admin) y devuelve `checkout_url` de Stripe (reutiliza `_send_sepa_link`, mode=setup, 0 € hoy). Al completar el mandato, `_on_card_saved` graba `recurring{method:sepa,last4,savedPm}`.
- **Cobro mensual automático con SEPA**: `monthly_billing_job` ahora es method-aware (`_get_saved_pm_typed`): tarjeta→cobro inmediato (paid); SEPA→PaymentIntent off_session `sepa_debit`→status "processing" (factura chargeStatus=processing + stripePaymentIntentId, se cierra por webhook `payment_intent.succeeded`). Resultado incluye `processing`.
...
### Iteración 2026-06 (fork) — FIX facturas duplicadas (idempotencia por periodo) + limpieza
- **CAUSA RAÍZ**: la clave de idempotencia de la facturación mensual usaba `run_key = now.strftime("%Y-%m")` (mes en que CORRE el job), no el periodo facturado. Al forzar la facturación de julio en julio y otra vez en agosto (más una versión antigua sin billingRunKey), se generaban varias facturas del mismo cliente y periodo. Se veían 67 facturas con 44 duplicadas (12 grupos).
- **FIX (previene futuros dups)** en `monthly_invoicing_job` y `monthly_billing_job`: `run_key = p_start.strftime("%Y-%m")` (periodo facturado, estable) y la comprobación de existencia ahora es por PERIODO: `find_one({fiscalId, period: billed_period, kind: {\$ne:"service"}})`. Así, exista o no la factura, nunca se crea una segunda del mismo periodo (las de servicios puntuales kind="service" no se ven afectadas).
- **LIMPIEZA (dedupe)**: nuevo endpoint admin `POST /api/billing/dedupe-invoices?apply=false|true` (require_admin). Agrupa facturas mensuales por (fiscalId, period), conserva 1 (prioriza pagada / más reciente) y borra el resto. apply=false = simulacro; apply=true = ejecuta. Botón en Cobros "Limpiar facturas duplicadas" (data-testid `dedupe-invoices-btn`) que hace simulacro, confirma (avisando si hay pagadas entre las que se borran) y aplica.
...
### Iteración 2026-06 (fork) — FIX "Pagos recibidos" no mostraba SEPA ni pendientes
- **CAUSA**: `GET /api/admin/payments` leía SOLO `payment_transactions` (sesiones Stripe Checkout) filtrando payment_status=paid. Los cobros off_session (SEPA/tarjeta vía charge-now/charge-pending/charge-all/monthly job) y los adeudos SEPA en proceso solo actualizan `invoices`, y los pendientes tampoco estaban → no aparecían.
- **FIX backend**: el endpoint ahora se construye sobre `invoices` (fuente de verdad): devuelve todas con {status: paid|processing|pending|failed, method: sepa|card|None, customerName (join customers), amount, concept/period, paidAt}. `processing` = adeudo SEPA liquidando. Además MERGEA las `payment_transactions` pagadas cuyo invoice_number no esté ya (pagos públicos "Pagar cuenta"/SIM). Orden por fecha desc.
